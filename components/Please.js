  ...
  function define_Please() {
      ...
      switch (format_string) {
        ...
        case "rgb-string":
          for (i = 0; i < array.length; i++) {
            ...
            array[i] =
              "rgb(" + raw_rgb.r + "," + raw_rgb.g + "," + raw_rgb.b + ")";
          }
          break;
        case "hsv":
          break;
    ...
    function RC4Random(seed) {
      ...
      function get_random_byte() {
        ...
        return key_schedule[
          (key_schedule[key_schedule_i] + key_schedule[key_schedule_j]) % 256
        ];
      }
    ...
    Please.make_color = function (options) {
      ...
      if (color_options.base_color.length > 0) {
        ...
        if (
          color_options.base_color.match(/^#?([0-9a-f]{3})([0-9a-f]{3})?$/i)
        ) {
      ...
      for (var i = 0; i < color_options.colors_returned; i++) {
        ...
        if (base_color !== null) {
          hue = clamp(
            random_int(base_color.h - 5, base_color.h + 5, randomiser),
            0,
            360
          );
    ...
    Please.make_contrast = function (HSV, options) {
      ...
      if (contrast_options.golden === true) {
        ...
        var contrast_base = Please.make_scheme(HSV, {
